...
export const getReceivePaymentLinksByUserID = (params) => async (dispatch) => {
  try {
    ...
    const res = await api.get(`/api/admin/help-link/approve-pending`, config);
...
// Edit Help Link
export const editHelpLink =
  (help_link_id, user_id, navigate) => async (dispatch) => {
    dispatch(removeErrors());
    try {
      const config = {
        headers: {
...
export const getDownlinePendingLinks = (params) => async (dispatch) => {
  try {
    ...
    const res = await api.get(`/api/admin/help-link/all-pending-links`, config);
